...
<div class="chains view large-9 medium-8 columns content">
    <h3><?= h($chain->name) ?></h3>
    <table class="vertical-table">
        ...
        <tr>
            <th scope="row"><?= __('Local Chain') ?></th>
            <td><?= $chain->local_chain ? __('Yes') : __('No'); ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Venues') ?></h4>
        <?php if (!empty($chain->venues)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($chain->venues as $venues): ?>
            <tr>
                <td><?= h($venues->id) ?></td>
                <td><?= h($venues->name) ?></td>
